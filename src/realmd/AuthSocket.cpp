        ...
        if (result)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    srp->SetvAsHexStr(databaseV.c_str());
                    srp->SetsAsHexStr(databaseS.c_str());
                    BigNumber s;
                    ...
                    pkt.append(srp->GetBAsByteArray(32), 32);      // 32 bytes
                    pkt << uint8(1);
                    pkt.append(srp->GetgAsByteArray(), 1);
                    pkt << uint8(32);
                    pkt.append(srp->GetNAsByteArray(32), 32);
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if(!srp->CalculateSField(lp.A, 32))
        return false;

    srp->CalculateKField();
    srp->CalculateMField(_login);

    ///- Check if SRP6 results match (password is correct), else send an error
    if (!srp->CompareM(lp.M1, 20))
    {
        ...
        const char* K_hex = srp->GetKAsHexStr();
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    Field* fields = result->Fetch();
    srp->SetKAsHexStr(fields[0].GetString());
    delete result;
